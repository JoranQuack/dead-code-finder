...
export async function installVulture(): Promise<boolean> {
    try {
        ...
        vscode.window.showErrorMessage(
            `Error installing Vulture: ${(error as Error).message
    ...
    for (const line of lines) {
        ...
        if (match) {
            ...
            if (path.isAbsolute(filePath)) {
                ...
                // Try to resolve relative path against workspace
                const candidatePath = path.resolve(workspacePath, filePath);

                // Check if the resolved path exists
                if (fs.existsSync(candidatePath)) {
                    resolvedFilePath = candidatePath;
                } else {
                    // The relative path might contain the workspace folder name somewhere in it
                    // (e.g., "code/ai-tooling/agents/..." when workspace is "/home/user/code/ai-tooling")
                    // This can happen if vulture was run from a parent directory
                    const normalizedFilePath = path.normalize(filePath);
                    const workspaceFolderName = path.basename(workspacePath);

                    // Look for the workspace folder name anywhere in the path
                    const folderNameIndex = normalizedFilePath.indexOf(workspaceFolderName + path.sep);
                    const folderNameIndexUnix = normalizedFilePath.indexOf(workspaceFolderName + '/');
                    const foundIndex = folderNameIndex !== -1 ? folderNameIndex : folderNameIndexUnix;

                    if (foundIndex !== -1) {
                        // Extract the path after the workspace folder name
                        const afterFolderName = normalizedFilePath.substring(foundIndex + workspaceFolderName.length + 1);
                        const newCandidate = path.join(workspacePath, afterFolderName);

                        if (fs.existsSync(newCandidate)) {
                            resolvedFilePath = newCandidate;
                        } else {
                            // Try resolving from the parent of workspace
                            const parentDir = path.dirname(workspacePath);
                            const fromParent = path.resolve(parentDir, normalizedFilePath);
                            if (fs.existsSync(fromParent)) {
                                resolvedFilePath = fromParent;
                            } else {
